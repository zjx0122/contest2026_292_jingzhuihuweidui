...
#include <arch/board/stm32f407_health.h>

#ifdef CONFIG_STATUS_LED
#include "stm32_led.h"
#endif

/* Conditional logic in stm32f407_health.h will determine if certain features
...
int stm32_bringup(void)
{
  ...
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: bsp_led_init() failed: %d\n", ret);
    }
#endif
  ...
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: stm32_esp32s3_setup() failed: %d\n", ret);
    }
#endif

#ifdef CONFIG_SENSORS_SHT30
  ret = stm32_sht30_setup();
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: stm32_sht30_setup() failed: %d\n", ret);
    }
#endif

#ifdef CONFIG_SENSORS_VL53L0X
  ret = stm32_vl53l0x_setup();
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: stm32_vl53l0x_setup() failed: %d\n", ret);
    }
#endif

#ifdef CONFIG_SENSORS_LD2410B
  ret = stm32_ld2410b_setup();
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: stm32_ld2410b_setup() failed: %d\n", ret);
    }
#endif

#ifdef CONFIG_LCD_GC9A01
  ret = stm32_gc9a01_setup();
  if (ret < 0)
    {
      syslog(LOG_ERR, "ERROR: stm32_gc9a01_setup() failed: %d\n", ret);
    }
#endif
